const callback = () => {
  ...
  const menuChange = () => {
    const bannerTextTop = document.getElementsByClassName("banner-text")[0].offsetTop;
    const currScrollTop = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop;
    let menu = document.getElementsByClassName("menu")[0];

    if (currScrollTop > bannerTextTop) {
      menu.style.top = "-100%";
    }

    if (currScrollTop < lastScrollTop && currScrollTop > bannerTextTop) {
      menu.style.top = "0";
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (msg.value.length === 0 || msg.value.length > 751) {
